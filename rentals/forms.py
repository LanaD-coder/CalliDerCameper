...
class BookingForm(forms.ModelForm):
    ...
    primary_driver_name = forms.CharField(
        required=False,
        label=_("Name")
    )
    primary_driver_street_name = forms.CharField(
        max_length=100,
        required=False,
        label=_("Street Name")
    )
    primary_driver_street_number = forms.CharField(
        max_length=10,
        required=False,
        label=_("Street Number")
    )
    primary_driver_postal_code = forms.CharField(
        max_length=20,
        required=False,
        label=_("Postal Code")
    )
    primary_driver_town = forms.CharField(
        max_length=100,
        required=False,
        label=_("Town/City")
    )
    primary_driver_country = forms.CharField(
        max_length=100,
        required=False,
        label=_("Country")
    )
    ...
    additional_driver_email = forms.EmailField(
        required=False,
        label=_("Additional Driver Email")
    )
    additional_driver_street = forms.CharField(
        required=False,
        label=_("Street")
    )
    additional_driver_postal_code = forms.CharField(
        required=False,
        label=_("Postal Code")
    )
    additional_driver_town = forms.CharField(
        required=False,
        label=_("Town/City")
    )
    additional_driver_country = forms.CharField(
        required=False,
        label=_("Country")
    )
    ...
    deposit_hidden = forms.BooleanField(
        widget=forms.HiddenInput(),
        initial=True
    )

    additional_services = forms.ModelMultipleChoiceField(
        queryset=AdditionalService.objects.all(),  # pylint: disable=no-member
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.user and self.user.is_authenticated:
            self.fields['primary_driver_name'].initial = (
    ...
    def save(self, commit=True):
        ...
        booking.primary_driver_name = self.cleaned_data.get(
            'primary_driver_name', ''
        )
        booking.primary_driver_street_name = self.cleaned_data.get(
            'primary_driver_street_name', ''
        )
        booking.primary_driver_street_number = self.cleaned_data.get(
            'primary_driver_street_number', ''
        )
        booking.primary_driver_postal_code = self.cleaned_data.get(
            'primary_driver_postal_code', ''
        )
        booking.primary_driver_town = self.cleaned_data.get(
            'primary_driver_town', ''
        )
        booking.primary_driver_country = self.cleaned_data.get(
            'primary_driver_country', ''
        )

        booking.additional_driver_street = self.cleaned_data.get(
            'additional_driver_street', ''
        )
        booking.additional_driver_postal_code = self.cleaned_data.get(
            'additional_driver_postal_code', ''
        )
        booking.additional_driver_town = self.cleaned_data.get(
            'additional_driver_town', ''
        )
        booking.additional_driver_country = self.cleaned_data.get(
    ...
    class Meta:
        ...
        fields = [
            'start_date', 'end_date',
            'primary_driver_name', 'primary_driver_street_name',
            'primary_driver_street_number', 'primary_driver_postal_code',
            'primary_driver_town', 'primary_driver_country',
            'additional_driver_name', 'additional_driver_email',
            'additional_driver_contact_number', 'additional_driver_street',
            'additional_driver_postal_code', 'additional_driver_town',
            'additional_driver_country', 'deposit_hidden',
            'additional_services', 'pickup_location', 'pickup_time',
            'dropoff_location', 'dropoff_time', 'customer_notes',
...
class ReturnChecklistForm(HandoverChecklistForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if handover_instance:
            self.fields['initial_odometer'].initial = (
                handover_instance.odometer
            )
...
class HandoverChecklistSignatureForm(forms.ModelForm):
    signature_data = forms.CharField(
        widget=forms.HiddenInput(),
        required=False
    )
